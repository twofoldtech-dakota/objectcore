...
| 010 | Telemetry `POST /v1/events` + authenticated `/v1/events/stats` (D4 half 1) | P2 | S–M | Stage 3 | BUILT on `feat/telemetry-events`: `EventSink` port + pure `parseEvent`/`aggregateEvents` (registry-core), `LibSqlEventStore`/`InMemoryEventStore` + append-only `events` table (registry-db), sink+token-gated routes wired in `prod.ts`. Additive, never touches `deriveCatalog`. |
| 011 | OIDC publish `POST /v1/plugins` — self-service publishing (D4 half 2) | P2 | M | Stage 3, Stage 2 provenance gate | BUILT on `feat/telemetry-events` (same PR as 010): `OidcVerifier` port (`Mock` + real `GitHubOidcVerifier` RS256/JWKS via Web Crypto, no dep), pure `authorizePublish`/`parsePublish`, provenance gate re-enforced (`requiresProvenance`), writes via `CatalogStore` port. Inert until `OBJECTCORE_OIDC_AUDIENCE` set. |
| 012 | **Epic: design-system authoring meta-plugin** — `@objectcore/design` engine (DTCG token SSOT → derive/validate/gate) + `design-forge` plugin; dogfood to produce ObjectCore's marketing-site design system | P2 | L (P0–P6) | Stage 1 patterns (forge engine + eval judge) | **EPIC COMPLETE (P0–P6)** — research-backed (two `/deep-research` passes 2026-06-26); full worklist in `012-design-system-authoring-plugin.md`. `packages/design` (DTCG `validateTokens`/`resolveAliases`; `deriveDesignSystem` seam + theme resolver + Token Source/CssVar/Json sinks; WCAG/type/spacing/fluid/motion gate + advisory APCA; `DesignJudge` Mock/Anthropic + `runDesignEval`; `scaffoldDesignSystem` accessible-by-construction; 61 pkg tests). `plugins/design-forge` (`/design` + 3 skills + evals → catalog 13 plugins). `design:scaffold`/`design:check`/`design:build` CLIs; `design:check` wired into `bun run check`. **Dogfooded → `design/objectcore/` SSOT (indigo dev-tool brand, light+dark), gated GREEN.** Built on `chore/registry-domain-live`, uncommitted. Follow-ups: SessionStart foundations hook, Tailwind `@theme` sink, CI judged eval. Never touches `deriveCatalog`/the seam. |
| 014 | **Seeded design themes + widened role contract + generated spec pages** — `packages/design/presets/{inkwell,cathode}` (curated, AAA-verified by the gate's own math), `design:seed` quick-start CLI, `roles.ts`/`proof.ts` contract+proof (gate ≡ proof), `SpecHtmlSink` specimen pages, scaffold widened to the full contract, `design/objectcore` migrated (18 → 58 gated pairs), `/design` quick-start fork + `choosing-a-seeded-theme` skill | P2 | L | 012 | **BUILT** (branch `feat/014-seeded-design-themes`, agent DAG: foundation → 4 reviewed lanes → integration; worklist in `014-seeded-design-themes.md`) |
| 013 | **Epic: KB / agent-memory upgrade** — `@objectcore/knowledge` gains lifecycle (update/supersede/deprecate, active-only INDEX), staleness verification, deterministic lexical retrieval (+ offline retrieval evals in the gate), write-time dedup, usage/ROI tracking, a prose/dogfood pass, and the **MCP resource server** access seam (`packages/knowledge-mcp`, the only `@modelcontextprotocol/sdk` dependent) | P1 | L (WP1–7, each independently shippable) | 008/F1–F4 (built) | **DONE (all merged 2026-07-02)** — plan #30; WP1 lifecycle #31; WP2 staleness #32; WP3 retrieval #33; WP4 dedup #34; WP5 usage #35; WP7 MCP server #36; MCP `kb_add`-dedup + `kb_cite` integration #37; WP6 prose/docs/changesets #38. Executed as an agent DAG (WP1 → WP2∥WP3 → WP4∥WP5∥WP7 → WP6), gate-green PR + human review per WP. Market-gap research 2026-07-02; full worklist in `013-kb-agent-memory-upgrade.md`. |
...
- **D4 — telemetry (`/v1/events`) + OIDC publish (`POST /v1/plugins`)**: **BOTH BUILT**
