...
| 009 | **F7** — recursive self-improvement of the forge engine, strictly eval-gated (design + Phases 0–2 + OQ4) | P1 | S (design) + M×3 | 008/F4 | **Phases 0–2 BUILT/merged** (PR #14): default-deny boundary, golden + guard meta-eval corpus, `forge-improver` proposer + admission pipeline (`forge:improve`), declared-improvability backlog (`forge:suggest`). **OQ4 measurement primitive BUILT** on `feat/oq4-eval-score`: graded gate-health score (`score.ts` → `dist/eval-score.json`) + non-regression admission. Remaining frontier: OQ4 longitudinal (score history) + Phase 3 autonomous executor. |
