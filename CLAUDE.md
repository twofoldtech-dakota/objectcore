...
- **Additive routes behind the seam.** Built now: **search** (`GET /v1/search?q=&keyword=&category=`,
  ...
  **token-gated** (`OBJECTCORE_EVENTS_TOKEN` — inert/open until set, the `deploy.yml` posture);
  ...
  `provenance` column). Sink-gated + **inert until `OBJECTCORE_OIDC_AUDIENCE` is set**. The live
  verifier's network path is untested (like `AnthropicJudge`); the pure authz/parse/gate logic is
  fully gate-tested. `/v1/marketplace.json` stays frozen.
