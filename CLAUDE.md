...
- The engine (`packages/forge/src/scaffold.ts`) takes a **`PluginSpec`** (the output of grill+plan) and emits the plugin dir, components (`skills/`, `commands/`, and — since F2/F3/F5 — `hooks/hooks.json`, `agents/*.md`, and `.mcp.json`), and `evals/` specs. It guards the hard rules at write time (kebab-case, components at root, string `repository`, array `keywords`), **refuses to emit a skill without activation cases** or (F4) **an agent without delegation cases** — cross-checking each case names a declared skill/agent — validates a `hooks` spec (known events, valid action types; owns the `{ "hooks": {...} }` wrapper), validates `agents` — **rejecting the forbidden `hooks`/`mcpServers`/`permissionMode` fields** and serializing tool lists comma-separated (the array form has a spawn bug) — and (F5) validates an **`mcp`** spec (per-server transport stdio/http/sse with the right required field; owns the `{ "mcpServers": {...} }` wrapper, emits `.mcp.json` at the ROOT so server objects stay out of the manifest). A plugin may be hooks-, agents-, or MCP-only (no skill/command). It never overwrites without `force`. (F4 closed the agent-delegation gap; F5 makes MCP forge-generatable.)
