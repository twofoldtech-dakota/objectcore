...
- **`RegistryDbSource`/`RegistryDbSink`** are ports in the pure core (`sources.ts`/`sinks.ts`,
  ...
  `release:publish` derives the pinned catalog; the live backend is updated over OIDC by
  `registry:publish` (the CI path — writes through the `CatalogStore` via `POST /v1/plugins`), while
  `registry:ingest` (the same `RegistryDbSink`, direct via `DATABASE_URL`) remains as manual
  break-glass. Either way the server reads rows back through `RegistryDbSource` and re-runs the same
  `deriveCatalog`. The DB stores RAW manifests + pin coordinates, never finished entries — one
  derivation path holds.
...
- **Hosting: Fly.io** (`Dockerfile`, `fly.toml`) runs the Bun + Hono entry unchanged; secrets
  `DATABASE_URL` + `TURSO_AUTH_TOKEN`; `registry.objectcore.ai` via `flyctl certs add` (a CNAME to
  `objectcore-registry.fly.dev`; apex/`www` stay on Vercel for the marketing site). `deploy.yml` deploys
  on merge to main, **inert until `FLY_API_TOKEN` is set**. Release CI runs `registry:publish` (OIDC)
  so the live backend updates with no redeploy; `registry:ingest` (direct DB) stays as break-glass.
- **Additive routes behind the seam.** Built now: **search** (`GET /v1/search?q=&keyword=&category=`,
  ...
  **token-gated** (`OBJECTCORE_EVENTS_TOKEN` — inert/open until set, the `deploy.yml` posture);
  plus an **authenticated** `GET /v1/events/stats` (`aggregateEvents`/`EventSink.stats()`,
  always token-gated). It never touches `deriveCatalog`; and **OIDC publish** (`POST /v1/plugins`,
  plan 011) — the HTTP analogue of the release pipeline: an `OidcVerifier` port
  (`MockOidcVerifier` for tests, `GitHubOidcVerifier` = real RS256-JWT-over-JWKS via Web Crypto,
  no dep, fail-closed), pure `authorizePublish` (issuer/audience/allowlisted-repo, fail closed on a
  missing repo claim) + `parsePublish` (reuses `validateSchema`; `ref` recomputed via `releaseTag`,
...
- `.claude/settings.json` is the ARMED dogfood wiring for the factory's own agentic loop: the kb-writer + reflection plugin hooks run directly in sessions on this repo (KB index at session start, reflection nudge on a red gate, lesson-capture prompt on stop) — the same hooks the plugins ship to consuming projects via the catalog.
